<template>
  <ExpandableAside :order="1000" v-if="!error" :visible="!cytoscapeError">
...
const { data, error } = await useFetch("/api/manifest", {
  server: true,
  lazy: false,
});
...
watch([graphEl, data], ([graphEl, data]) => {
  ...
  if (graphEl != null && data != null) {
    try {
      cytoscapeError.value = undefined;
      cy.value = cytoscape({
        container: graphEl,
        elements: {
          nodes: [
